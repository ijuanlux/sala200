...
class Room {
    ...
    constructor(domain, game_id, sessionid, name, max, current, password, userid, socket, extra, coreVer) {
        ...
        /** @type string */
        this.password = String(password == null ? '' : password).trim();
        /** @type boolean */
    ...
    checkPassword(password) {
        // El cliente puede no mandar password (undefined): jamas debe tirar el servidor.
        return String(password == null ? '' : password).trim() === this.password;
    }
    /**
     * Adds the user
     * @param {User} user 
